...
{
    $pdo=db();if(!$pdo)return [];$q=trim((string)($_GET['q']??''));$categoryId=(int)($_GET['categoria']??0);$brandId=(int)($_GET['marca']??0);$familyId=(int)($_GET['familia']??0);$modelId=(int)($_GET['modelo']??0);$params=[];$where=["v.status='publicado'",library_video_access_sql($params)];
    ...
    return compact('videos','categories','brands','families','models','featured','grouped','q','categoryId','brandId','familyId','modelId');
}
